...
class chargify {
	...
	function subscriptionCreate()
	{
		...
		if(isset($_GET["customer_reference"]) && isset($_GET["subscription_id"]) && !isset($_REQUEST["chargify.subscriptionPost"]))
		{
			...
			$opt = array("api_key" => $d["chargifyApiKey"],"test_api_key" => $d["chargifyTestApiKey"],"domain" => $d["chargifyDomain"],"test_domain" => $d["chargifyTestDomain"],"test_mode"=>($d["chargifyMode"] == 'test'? TRUE : FALSE));	
			$connector = new ChargifyConnector($opt);
			$sub = $connector->getSubscriptionsBySubscriptionId(htmlspecialchars($_GET["subscription_id"]));
			$subid = (htmlspecialchars($_GET["subscription_id"]));
			if($sub->getState() == 'active' || $sub->getState() == 'trialing')
			{
				$customer = $connector->getCustomerByReference(htmlspecialchars($_GET['customer_reference']));
				$customer_email = $customer->getEmail();
				$trans = get_transient('chargify-'.md5($customer_email));

				if(is_array($trans) && isset($trans['user_email']))
				{
					$email = $trans['user_email'];
					$user_pass = $trans['user_pass'];
					if (isset($trans['user_login'])) {
						$user_login = $trans['user_login'];
					} else {
						$user_login = $trans['user_email'];
					}
					...
					else
					{
						...
						$customer = $connector->getCustomerByReference($_GET['customer_reference']);
                                		$customer_email = $customer->getEmail();
						delete_transient('chargify-'.md5($customer_email));
						...
						update_usermeta( $user_id, 'first_name', $sub->getCustomer()->getFirstName());
						update_usermeta( $user_id, 'last_name', $sub->getCustomer()->getLastName());
						...
						else
						{
							...
							self::login( $user_id, $email,$trans['return_url']."?customer_id=".$sub->getCustomer()->getId()."&customer_reference=".$sub->getCustomer()->getReference()."&subscription_id=".$subid);
						}
					}
				}
				else
				{
					//echo '<script>console.log("Unknown customer ref");</script>';
				}
	...
	function subscriptionRedirect()
	{
		...
		if ( wp_verify_nonce( $_POST['chargify_signup_noncename'], plugin_basename(__FILE__) ) && is_numeric($_POST["submit"]))
		{
			...
			{
				$_POST["chargify_signup_error"] = array('ERROR'=>"<span class=\"chargify-error\">All fields are required. Please enter a name and valid email address.</span>");
				return 0;
			}
			...
			if (strpos($user_login, '@') || strpos($user_login, '_')) {
				$_POST["chargify_signup_error"] = array('ERROR'=>"<span class=\"chargify-error\">You cannot have an @ symbol or an underscore in your username.</span>");
				return 0;
			}

			if(email_exists($user_email) && !$current_user->ID)
			{
				$_POST["chargify_signup_error"] = array('ERROR'=>"<span class=\"chargify-error\">That email address is already in use, please choose another.</span>");
				return 0;
			}
			elseif(username_exists($user_login) && !$current_user->ID)
			{
				$_POST["chargify_signup_error"] = array('ERROR'=>"<span class=\"chargify-error\">That user name is already in use, please choose another.</span>");
				return 0;
			}
			else
			{
				...
				if(!$user_pass) {
					$_POST["chargify_signup_error"] = array('ERROR'=>"<span class=\"chargify-error\">Password mismatch, please try again.</span>");
					return 0;
				}
	...
	function subscriptionPost($content)
	{
		...
		if ( wp_verify_nonce( $_POST['chargify_signupcc_noncename'], plugin_basename(__FILE__) ))
		{
			...
			{
				$_POST["chargify_signup_error"]['ERROR'] = '<span class=\"chargify-error\">That email address is already in use, please choose another.</span>';
				return 0;
			}
			else
			{
	...
	function controlForm()
	{
        ...
        if (isset($_POST['chargify-save-nonce']) && wp_verify_nonce($_POST['chargify-save-nonce'], plugin_basename(__FILE__)))
		{
			...
			$d["chargifyDefaultNoAccess"] = stripslashes($_REQUEST["chargifyDefaultNoAccess"]);
			...
			if(isset($_REQUEST['chargifyproduct']) && is_array($_REQUEST['chargifyproduct']))
			{
				...
				foreach($_REQUEST['chargifyproduct'] as $k=> $v)
				{
					if($v['enable'] == 'on')
					{
						$product = $connector->updateProduct($k, array('return_url'=>$signupurl,'accounting_code'=>$v['acctcode'],'description'=>stripslashes($v['description']),'name'=>$v['name']));
        ...
        echo '<div class="wrap">';
			...
			foreach($products as $p)
			{
				...
				echo '<div>Description<br><textarea name="chargifyproduct['.$p->id.'][description]">'.str_replace('\\','',$p->getDescription()).'</textarea></div>';
